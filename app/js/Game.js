...
define(['underscore', 'backbone', 'io', 'Alien', 'Astronaut', 'ScoreDisplay', 'LifeBar', 'ModalWindow', 'easel', 'preload', 'sound', 'tween'], function(_, Backbone, io, Alien, Astronaut, ScoreDisplay, LifeBar, ModalWindow){
	...
	var GameView = Backbone.View.extend({
		...
		// Hardware Push button locations
		push_buttons: [
			{x: 290, y: 480, pin: 11},
			{x: 710, y: 530, pin: 10},
			{x: 1040, y: 465, pin: 9},
		...
		initialize: function(){
		    ...
		    socket.on('btnPress', function(data){
		      ...
		      for(var i=0; i < scope.push_buttons.length; i++){
		      	var button = scope.push_buttons[i];

		      	if(button.pin === data.button){
		      		//console.log('proiedades del botón: '+button.pin+'  =  '+button);
		      		//console.log(data.button)

		      		var astronaut = scope.astronaut;

		      		var a = button.x - astronaut.sprite.x;
		      		var b = button.y - astronaut.sprite.y;
		      		...
		      		for(var j = 0; j < scope.enemies.length; j++){
			      		var enemy = scope.enemies[j];

			      		var a = button.x - enemy.sprite.x;
			      		var b = button.y - enemy.sprite.y;
			      		var distance = Math.sqrt( a*a + b*b );
		...
		playNextLevel: function(){
			...
			this.enemies_simultaneous += (this.level % 3 === 0) ? 1 : 0;
